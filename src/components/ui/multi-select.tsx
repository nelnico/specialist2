...
export function MultiSelect({
  ...
  const clearAll = useCallback(() => {
    ...
    if (!isControlled) setUncontrolledSelected(empty);
    notifyChange(empty);
...
export function MultiSelectValue({
  ...
  const checkOverflow = useCallback(() => {
    ...
    const nodes = containerElement.querySelectorAll<HTMLElement>(
      "[data-selected-item]"
    );

    if (overflowElement != null) overflowElement.style.display = "none";
    nodes.forEach((child) => child.style.removeProperty("display"));
    let amount = 0;
      ...
      {[...selectedValues]
        ...
        .map((value) => (
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <XIcon className="size-2 text-muted-foreground group-hover:text-destructive cursor-pointer" />
            )}
          </Badge>
        ))}
      <Badge
