...
interface SpecialistSearchSelectProps<N extends SelectArrayField> {
...
export function SpecialistSearchSelect<N extends SelectArrayField>({
  control,
  name,
  label,
  options,
    ...
    <FormField
      ...
      render={({ field }) => {
        ...
        if (multiple) {
          ...
          // SINGLE SELECT MODE
          const value = currentNumbers[0] ? String(currentNumbers[0]) : "";

          const handleChange = (val: string) => {
            const num = Number(val);
            field.onChange(Number.isFinite(num) ? [num] : []); // still store as number[]
          };
...
export default SpecialistSearchSelect;
